...
log "[1/10] manifest_paths — пути из update-manifest.json в дереве..."
...
log "[2/10] seed_references — protocol-*.md ссылки на seed/ существуют..."
...
log "[3/10] extension_table — extensions/README.md table ↔ EXTENSION POINT в protocol-*.md/SKILL.md..."
EXT_VIOLATIONS=0
...
log "[4/10] hook_artifact — hooks не грепают TOOL_INPUT (R4.5 антипаттерн)..."
HOOK_VIOLATIONS=0
...
log "[5/10] runner_readonly — runners резолвят prompts/role.yaml/notify через \$IWE_TEMPLATE..."
RUNNER_VIOLATIONS=0
...
log "[6/10] install_failfast — install.sh имеют grep '{{' check на PLIST_SRC..."
...
log "[7/10] prompts_python_shell_coverage — нет hardcoded DS-strategy в prompts/.py/.sh..."
...
log "[8/10] sed_placeholder_escape — substituted runners НЕ имеют bare {{X}} в sed (build-runtime подменит)..."
SED_VIOLATIONS=0
...
log "[9/10] manifest_version — update-manifest.json version == CHANGELOG.md version..."
...
log "[10/10] deprecated_runner_usage — deprecated_files не содержат промптов, которые runner ещё вызывает..."
