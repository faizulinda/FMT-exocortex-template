...
- **Система-в-роли ≠ Роль** (FPF A.2.1, R5). Работу выполняет система-в-роли; роль лишь задаёт метод и контекстное участие. Нотация: `система#роль:контекст`. Роль сама ничего не делает.
- **Целевая система проекта ≠ класс систем** (FPF A.2.1, R5). Целевая система проекта — роль, которую некоторая система играет в конкретном проекте. Одна физическая система в разных проектах = разные роли. Тест: «это класс вещей или функциональное место в контексте?»
- **Сервис ≠ метод** (FPF A.6.8, R5). Сервис = обещание + провайдерская роль + метод оказания + работа оказания + свидетельства результата. Метод — один компонент сервиса. «Метод оказания» ≠ сервис целиком.
- **Мастерство (роль) ≠ Степень мастерства (характеристика)** (FPF A.2, R9). Мастерство::роль = функциональная роль части организма/экзокортекса в деятельности. Степень мастерства = измеримая характеристика качества исполнения. Тест: «это кто исполняет или насколько хорошо?»
