...
### Напоминания через Telegram (S-44)

**Триггеры:** «напомни через X», «поставь таймер», «remind me in X» — любой запрос на напоминание с временным интервалом.

**Правила:**
- При запросе напоминания → использовать `send_telegram_message` с `schedule_at` (рассчитать время = сейчас + интервал в ISO 8601). Одновременно поставить ScheduleWakeup как резервный канал.
- При срабатывании ScheduleWakeup → СНАЧАЛА `send_telegram_message` (немедленно, без `schedule_at`), ПОТОМ написать в чате.
- Если `send_telegram_message` вернул ошибку → диагностировать (проверить деплой бота), сообщить в чате с причиной.

**Зачем:** уведомление в чате IDE видно только если окно открыто; Telegram доставляет независимо от устройства и приложения.
